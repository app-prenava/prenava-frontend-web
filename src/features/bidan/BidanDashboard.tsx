...
import AgeAnalyticsCard from './AgeAnalyticsCard.tsx';
import GestationalAgeAnalyticsCard from './GestationalAgeAnalyticsCard.tsx';
...
export default function BidanDashboard() {
  ...
  const {
    data: ageAndGestationalResponse,
    isLoading: ageLoading,
    error: ageError,
  } = useQuery({
    queryKey: ['ageAndGestational'],
  ...
  const ageAnalytics = ageAndGestationalResponse
    ? (() => {
        console.log('[DASHBOARD] Computing age analytics');
        const result = aggregateAgeAnalytics(ageAndGestationalResponse);
        console.log('[DASHBOARD] Age analytics result:', result);
        return result;
      })()
    : undefined;
